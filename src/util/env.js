        ...
        for (let i = version.length - 1; i >= 0; i--) {
          if (!version[i]) {
            ...
            if (temp.length < 2) {
              ...
              weexVersion[temp[0].replace(/[\s|-]/g, '')] = temp[1].replace(/[\s|v]/g, '');
            }
          }
        }
        versionCache['weex'] = weexVersion;
        return callback({
          version: weexVersion
        });
      }
      else {
        const isNpm = command.indexOf('node') !== -1;
        versionCache[isNpm ? 'npm' : 'node'] = stdin.toString().split('\n')[0].replace('v', '').trim();
        return callback({
